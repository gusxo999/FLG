 ...
 *
 * 계단꼴 홉은 [모서리]에서 방향을 한 번 튼다. 그 모서리에서 **시작하는** 지하 점프는 다음
 ...
 * 붙잡는다.
 *
 * 그 가짜 점유의 대가가 **둘**이다: 다른 홉의 정당한 트랙과 부딪히고, 계획끼리의 상호 검사가
 * 서로를 이유로 **둘 다** 탐색 폴백시킨다. 지하 횡단은 애초에 그 홉을 **피해서** 밑으로
 * 건너려고 낸 계획인데, 검사기가 그걸 충돌로 읽은 것이다.
 *
 * 실측(브라우저 kr-glass 16머신)에서 홉 16개 중 5개가 탐색으로 났고, 그 조사에서 나온 버그다.
 */
...
const config: PackConfig = {
  ...
  // 예약 장부를 켠다 — 안 켜면 홉이 전부 dijkstra 로 나서 이 버그가 안 드러난다.
...
describe("지하 횡단 사슬 — 모서리 점프를 앞 구간이 먹지 않는다", () => {
  ...
  it("이 형상이 실제로 지하 횡단을 계획한다 (전제 확인)", () => {
    const kinds = [...(p.channelGeometry?.hops.values() ?? [])].map((g) => g.kind);
