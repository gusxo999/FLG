...
## 5. Underground belt / pipe 자동 사용 안 함

**우선순위: P1**

**증상:**
- 머신 사이 유일한 통로가 다른 unit 의 입출력 줄로 막혀 있으면 라우팅 실패 → `route-failed` warning.
- 사용자가 underground 를 선택했어도 사용되지 않음.

**원인:**
- router 가 4방향 인접 셀로만 BFS. 점프 액션 없음.

**해결 방향:**
1. router 에 "지하 점프" edge 추가: 같은 row/column 으로 max_underground_distance 이내 거리의 비-점유 셀로 가중치 1+jump_cost 로 진출.
2. underground 입구/출구 두 개의 셀을 한 entity 페어로 emit (direction = 진행 방향 / 그 반대).
3. fluid 도 마찬가지로 `pipe-to-ground` 변형.
...
| **P1** | §4 다중 합류 / §5 underground 자동 사용 |
