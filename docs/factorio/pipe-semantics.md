...
- 파이프 셀의 `direction` 은 **항상 0** 이다([cellBuilder.makePipeCell](../../frontend/src/autoLayout/util/cellBuilder.ts)).
...
면(E)을 보게 한다([fluidPorts.chooseMachineDirection](../../frontend/src/autoLayout/module/fluidPorts.ts)).
...
([module/pipeFlow.ts](../../frontend/src/autoLayout/util/pipeFlow.ts)).
...
- `pipe-to-ground` — **prototype 무관 전부** 간섭한다. 그래서 우리 모델은 지하 파이프를 단일
  `blockGroup = "pipe-to-ground"` 으로 묶는다([containerModel.UndergroundCorridor](../../frontend/src/autoLayout/containerModel.ts)).
