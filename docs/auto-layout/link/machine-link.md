...
### 한 칸만 내려간다 — **가장 적게 자르는 레인 하나**를 쓴다

배정은 후보(면 × 레인)마다 사유를 낸다. 그 **막힌 행을 합치면 안 된다.**

```
stone-tablet 의 사유    d2: 막힌 행 263개   ← 내 구간이 통째로 먹혔다
                       d3: 막힌 행 2개     ← 여기가 자를 자리다
합치면                 265개 경계 → 머신마다 한 토막 → **90토막**
```

**90토막은 사다리 3단(다이렉트)이다.** 1단을 하려다 3단으로 건너뛴 것이고, 포트가 1→90 이
되어 납품·채널이 폭발했다(2026-08-26 실측: 30초가 지나도 안 끝났다). 그래서 **`blockedRows`
가 가장 적은 후보 하나만** 쓴다 — 그게 *"한 칸만 내려간다"* 의 구현이다.

### 대가 — **트리마다 다르다. 그래서 기본이 꺼짐이다**

토막 하나가 **포트 하나**다. 포트가 늘면 납품 경로가 늘고, 납품 경로가 늘면 채널이 는다.
그 대가가 **자리를 얻는 값보다 큰 트리가 있다.**

> **실측 (2026-08-26, KR2+SE):**
>
> | 트리 | 사다리 끔 | 사다리 켬 | |
> |---|---|---|---|
> | `kr-glass` | 이슈 0 · 2.3초 | 이슈 0 · 2.1초 · 쪼갬 0 | 차이 없음 |
> | `battery` | 이슈 1 · 8.5초 | 이슈 1 · 8.5초 · 쪼갬 0 | 차이 없음 |
> | `electronic-circuit` | **이슈 2**(트리 거절) · 1.9초 | **이슈 0** · 3.8초 · 쪼갬 2 | **살아났다** |
> | `advanced-circuit` | 이슈 13 · 20초 | **이슈 49** · 50초 · 쪼갬 46 | **나빠졌다** |
>
> `electronic-circuit` 은 **정확히 의도대로** 됐다 — 한 줄이 세 토막이 되고 납품
> `planned 11 / dijkstra 0 / 실패 0`, 못 앉은 줄 0.
>
> `advanced-circuit` 은 46번 쪼개며 이슈가 13 → 49 로 늘었다. **쪼개도 안 앉을 줄까지
> 쪼갰기 때문이다** — 지금 코드는 *"막힌 행이 있으면 쪼갠다"* 이고, *"쪼개면 앉나"* 를
> 묻지 않는다.

**다음 한 걸음은 그 물음이다** — 쪼갠 결과가 실제로 앉는지 확인하고, 안 앉으면 **쪼개지 않는다**
(사다리는 후보를 늘리는 것이지 포트를 늘리는 것이 목적이 아니다). 그때까지 기본은 꺼짐이다.
