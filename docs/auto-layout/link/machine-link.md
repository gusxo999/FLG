...
B 로 S 를 자른 조각들의 구간이 B 와 안 겹치면  →  그 조각은 앉는다   ([nailsWorthCutting])
```
...
> 하나다. 그래서 [nailsWorthCutting] 은 두 행 목록만 받는다.
...
|---|---|---|---|
| `쪼갬` | 막힌 칸 **사이**에 내 좌석이 있다 | **1단** | ✅ 된다 — *켜진 채로 이게 보이면 분류가 아니라 **버그**다* |
| `포트막힘` | 벨트는 지나가는데 **포트 칸**이 막혔다 | 1단 | ⛔ **안 만들었다** — 쪼개면 진출 행이 옮겨가는데 [nailsWorthCutting] 이 아직 안 본다 |
| `좌석막힘` | 남의 **벨트**가 내 좌석 행을 덮었다 | 2단(gap)·3단(다이렉트) | ⛔ 칸 자체가 없다 |
| `좌석부족` | 면에 빈 좌석 **자체**가 모자란다 | 2단·3단 | ⛔ 칸 자체가 없다 |
