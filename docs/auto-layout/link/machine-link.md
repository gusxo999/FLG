...
한 면의 깊이는 `reach` 종류 수만큼(보통 둘)이고, **얕은 줄에 앉은 줄의 포트가 다음 깊이에
...
```
planModulePorts   배정이 못 앉는다 → **왜**를 낸다        DepthShortage { face, clusterBeltDepth, blockedRows }
      ↓ GeneratedModule.depthShortages (linkId → 사유)
...
못은 **한 깊이씩 깊어진다** — 깊이 `d` 의 포트는 `d+1` 에 선다. 가장 깊은 줄의 포트가
박는 못은 **깊이가 아닌 칸**에 서서 아무와도 안 다툰다. 그래서 깊이가 `R` 개면 `R−1`
단계에서 멈춘다. 지금 스펙(`R=2`)에선 **`d2` 줄의 포트만 못이 된다.**

### `span-blocked` 가 푸는 문제 — **막힌 칸 사이에 빈 자리가 있나**

배정은 후보(면 × 깊이)마다 사유를 낸다. **어느 후보를 자를지는 기하가 정한다:**
...
> **2026-08-26 에 여기를 *"막힌 행이 가장 적은 깊이"* 으로 골랐다가 데였다.** 포트는 점이고
> 벨트는 구간이라 개수가 갈릴 뿐이어서 **상관이지 원인이 아니었고**, 짧은 벨트에 막힌 깊이를
...
[DepthShortage] 에 있는데 후자를 아무도 안 읽고 있었다.

> 이름표는 **모듈 단위 집계**다. 줄 이름과 1:1 로 잇지 않는 것은 [GeneratedModule.depthShortages]
> 가 `linkId` 로만 묶여 있어서다 — **없는 대응을 지어내지 않는다.** 줄까지 짚어야 하면
> `flg.report()` 의 `면깊이` 아래 *못앉음* 줄들이 품목별 사유를 그대로 보여 준다.
...
> **남은 것:** gap 면당 그룹 **하나**(가로 벨트 한 줄)만 받는다 — 깊이 둘째 줄(긴팔)은 후속.
