...
### 대가 — **아직 안 재졌다. 그래서 기본이 꺼짐이다**

토막 하나가 **포트 하나**다. 포트가 늘면 납품 경로가 늘고, 납품 경로가 늘면 채널이 는다.

> **실측(2026-08-26):** `electronic-circuit` 을 사다리 켜고 돌리면 **3분이 지나도 안 끝난다**
> (끄면 수 초). `battery` 는 쪼갤 것이 없어 차이가 없었다(14초 ↔ 17초).
> **원인은 아직 안 갈랐다** — 후보 셋: ① 1차 생성을 한 번 더 하는 비용 ② 포트가 늘어난
> 납품·채널 비용 ③ 쪼갠 줄이 다시 못 앉아 사유만 쌓이는 것.
>
> **그 셋을 가르기 전에는 켜지 않는다.** 켜면 지금 되는 배치가 느려지고, 느려지는 이유를
> 모르면 고칠 수도 없다.
