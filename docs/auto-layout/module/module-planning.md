...
| 요소 | 무엇을 정하나 | 임의성 |
|---|---|---|
| 선호 면(출력 W · 입력 E) | 면의 1차 값 | 정책(고정) |
| **그룹 도착 순서** | 누가 먼저 앉나 → **누가 못을 박나** | ⚠️ **도착 순** — `groups.forEach` |
| 레인 후보 정렬(팔 적은 순 → 얕은 순) | 레인 깊이 | 정렬됨 ✔ (R2 지킴) |
| 빈 좌석을 앞에서부터 | 행 구간의 시작 | 결정적 ✔ |
| `ctx.ends` **선착순(N 먼저)** | **기둥 끝**(관통 그룹) | ⚠️ **도착 순 + 짝의 위치를 안 본다** |
| 넘침 면 순서(`["W","S","N"]` 등) | 면의 2차 값 | 정책(고정) |

### (다) 되먹임 — 배정 결과가 다시 입력이 된다

```
배정(plans) ─→ gapRowsFromPlans ─→ gap 폭 ─→ 머신 좌표 ─→ 포트 절대 좌표
배정(plans) ─→ moduleExtent ─→ tidy-tree topY ─→ lineEnds ─→ 2차 생성
laneShortages ─→ 사다리 ─→ linkCache ─→ **1차 재생성**
