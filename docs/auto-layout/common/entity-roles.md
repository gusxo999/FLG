...
**현재 알고리즘:** 머신 footprint 는 `tile_width × tile_height` 를 그대로 써 **가변 지원**(보일러 3×2, 사일로 9×9 등도 배치)되지만, **회전은 0(북쪽) 고정**이다. 회전·fluidbox 면 제약은 [known-limits §6](known-limits.md). 단, 모든 변환기는 렌더 `EntityType` 이 Assembler 로 단순 매핑된다([machinePlacer.ts](../../../src/autoLayout/shared/cells/place.ts) `machineEntityType`).
...
- throughput 은 `rotation_speed × stack_size` 로 결정. 인서터 처리량 모델은 [shared/arith/inserter.ts](../../../src/autoLayout/shared/arith/inserter.ts) 참조 — 사용자 override 우선.
...
- **underground-belt** (1×1 두 개): 입구·출구 페어. 사이는 다른 entity 가 통과 가능.
  **인서터는 입구·출구 칸에서 직접 집고 놓는다**(2026-08-14 사용자 확인) — 그 두 칸은 지표
  엔티티이고 지상 벨트와 같게 다뤄진다. 반대로 **사이(몸통)에서는 못 집는다** — 거기엔
  지표에 아무것도 없다. 이 비대칭이 배치를 하나 정한다: 벨트 한 줄이 남을 건너려고
  잠수할 때 **잃는 탭은 몸통 칸뿐**이고, 잠수 구간 `[t−1, t+1]` 이면 `t±1` 은 그대로 탭한다.
- **[[용어사전#splitter|splitter]]** (2×1): 두 입력 → 두 출력. 분배 / 우선순위 / 필터 가능.
- 진행 방향 = `direction` 필드. 라인 합류는 splitter 또는 측면 합류로만.

**두 lane 의 의미:** 게임상 한 belt 는 좌/우 두 lane 으로 서로 다른 두 item 까지 동시 운반 가능하다(줄이 45/s 면 **레인마다 22.5/s**). **현재 router 는 이를 활용하지 않는다** — occupancy 가 모든 belt 셀을 통과 불가로 처리해 라우팅끼리 벨트를 공유하지 못한다(belt mixing 미구현, [known-limits §5](known-limits.md)). 한 라우팅 = 한 컨테이너 = 한 belt 줄.

> **레인이 어떻게 정해지는가는 [[belt-lane-semantics]] 가 단일 출처다** — 인서터는 레인을 못 고르고(언제나 먼 레인), 레인을 가르는 것은 **합류 기하**다. 아래 흐름-인접 불변식이 막고 있는 그 기하가 곧 레인 합류이기도 하다.
...
이 불변식은 [containerRouting.ts](../../../src/autoLayout/shared/route.ts) 의 [[용어사전#collectBeltFlow|`collectBeltFlow`]]
...
> 아니라 **인접 자체**가 합류다. 그 무방향 버전의 가드가 [`collectPipeFlow`](../../../src/autoLayout/shared/pipeFlow.ts)
...
- 파이프는 머신 벽 아무 데나가 아니라 **유체 상자의 연결 칸**에만 붙는다 → 유체 줄의 면은 우리가 고르는
  게 아니라 머신이 정한다. 그래서 **머신을 돌린다**([fluidPorts.chooseFluidTrunkPlan](../../../src/autoLayout/module/gamedata.ts)).
...
- **합류 가드**: [`collectPipeFlow` / `PipeFlow`](../../../src/autoLayout/shared/pipeFlow.ts) — 다른 유체
