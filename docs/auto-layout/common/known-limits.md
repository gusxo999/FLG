...
## 11. 벨트 합류를 막는 판정이 **경로마다 따로** 있다 — 모듈 밖은 무방비

**우선순위: P2**

> **현황 조사:** [[belt-merge-guards]] — 방출기 넷 × 가드 셋의 실측표, **무엇을 켜면 무엇이
> 깨지는지**(§4), 합류의 절반이 **의도된 것**이라는 제약(§3). 이 항목을 고치기 전에 읽는다.

**원인 (하나):** 두 벨트는 타일을 안 나눠 써도 **흐름으로 이웃하면** 하나가 된다
([[belt-lane-semantics]] §3). 그 판정이 이 저장소에 **셋**으로 흩어져 있고, 셋이 서로를
모른다 — 그래서 어느 경로가 어느 경로를 막는지가 **경로 쌍마다 다르다.**

| 판정 | 어디에 걸려 있나 | 무엇을 막나 |
|---|---|---|
| `beltItems` + [[belt-terminus]] | 모듈 안쪽 방출 | 모듈 **자기** 벨트끼리의 끝-칸 합류 |
| `collectBeltFlow` / `beltFlowConflictCell` | **아무 데도** — 호출자 0 | (설계상) 라우팅끼리의 흐름-인접 합류 |
| `UndergroundCorridor` / `isJumpAllowed` | dijkstra 폴백 **한 곳** | 같은 티어 지하벨트의 페어링 절단 |
...
- 같은 이유로 **계획된 점프끼리도** 서로의 corridor 를 안 본다(오래된 구멍이다 —
  종착이 생기면서 노출면이 하나 늘었을 뿐이다).

**해결 방향:**
1. 판정을 **한 곳**으로 모은다 — `collectBeltFlow` 를 모듈 파이프라인의 납품·반출 방출
   앞에 걸고, 모듈 벨트도 그 입력에 넣는다(품목까지 보려면 `beltItems` 를 모듈 밖으로
   실어 올려야 한다 — `pipeCells` 가 유체에 대해 이미 하는 일과 같은 모양).
2. `plannedChainClear` 가 corridor 장부도 보게 한다. 지금은 칸(occupancy)만 본다.

**왜 지금 안 하나:** ①은 **끝점에서 죽을 수 있다**(납품의 끝은 정의상 모듈 트렁크와
흐름-인접이고, 탐색은 끝점 충돌을 못 피하면 `null` 을 낸다 → [[belt-merge-guards]] §4.1).
그래서 납품 경로가 통째로 폴백으로 밀릴 수 있어 **회귀 폭이 크고**,
②는 계획 단계가 corridor 를 아예 안 만드는 자리라 장부의 주인부터 정해야 한다.
종착의 노출은 **좁다** — 세 방향이 다 막혀야 서고(계측 `flg.report()` 의 `벨트끝 · 종착`),
그 사거리는 **가장 느린 티어**라 가장 짧다.

---

## 12. 반출 상자가 자기 모듈 몸통에 갇힐 수 있다 — 네 방향이 전부 막힌 포트

**우선순위: P2** (잠정 — 2026-09-12 등재, 관측 1건)

**증상:**
- `processing-unit` 트리(게임데이터 레시피 2,647 · `min` · 2026-09-11 브라우저 실행)에서 원료
  상자 하나(`n0-processing-unit-input-advanced-circuit-0`)가 외곽으로 못 나가 모듈 안에 남는다 —
  `반출 relocated 3 / skipped 1`. `flg.report()` 의 `반출배정` 줄에는 `막힘 1(후보0) · 행채널로도 불가 1`.
- 그 포트의 `wayOuts` 가 **비었다** — N·S·W·E 가 전부 자기 모듈 몸통에 막힌다. 행 채널에도
  못 들어가므로 통로를 늘려서는 못 구한다([[exit-ray]] §4 의 2홉을 지어도 마찬가지다).
...
**해결 방향:**
- 모듈 안에서 풀어야 한다 — `module/` 좌석 배정의 몫이다. 어떤 제약으로 풀지는 아직 안 정했다.
- 같은 면에 유체 줄이 둘 이상 서는 머신은 커스텀 레시피로 콘솔에서 만들 수 있다
  ([[custom-recipe]] — 유체 상자를 면·행으로 놓는다). 실데이터 없이 이 모양을 흔드는 길이다.
...
|----------|------|
| **P1** | §1 클러스터 형태 기둥 고정 |
| **P2** | §2 belt/pipe mixing · §3 아이템 머신 회전 미지원 · §9 Deprecated Dijkstra Guard(드래그) · §11 벨트 합류 판정 분산 · §12 반출 상자 갇힘(잠정) |
