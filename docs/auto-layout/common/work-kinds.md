...
| **D5** | **합류 여부를 계획과 방출이 다른 근거로 판정한다**(2026-09-12 대조 계측으로 발견) — 계획은 `seatOnSharedBelt` 가 성공했을 때만 공유하고 실패하면 짝을 푸는데, 방출은 `group.sharedLineId` 만 보고 **무조건** 먼저 나온 벨트에 얹는다. 결과는 둘이다: 아무도 안 쓰는 **유령 예약** 네 칸, 그리고 벨트 없는 칸에 선 팔 = **조용한 굶주림** | `emitInputLinks` 의 `reuse` · `planModulePorts:472` · 측정 `tempPlanDocs/구조-2축/1-도형-단일출처/` §2단계 F2 |
| **D6** | **합류 이끄는 줄의 기둥 밖 세 칸이 어느 장부에도 없다**(같은 계측 F1) — 비합류 경로는 기둥 밖 칸을 `ctx.outside` 에 올리는데 `mergeLead` 분기만 우회한다. 오늘은 그 장부가 대조만 하므로 피해가 없지만, 결정권을 주는 순간 결함이 된다 | `commitLinkFace` 의 `mergeLead` · `tempPlanDocs/구간-밖-주행/` A2 |
