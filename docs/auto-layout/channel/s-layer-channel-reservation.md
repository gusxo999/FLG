...
> [[channel-geometry-reservation]] 가 현행 단일 출처다.
>
...
> **관련 문서:** [.placement-search](../common/placement-search.md) — 배치/전략(§5.5 전략 등록부), [.entity-roles](../common/entity-roles.md), [.known-limits](../common/known-limits.md)
...
> 두 인접 [[용어사전#레이어 (layer)|레이어]] **사이에 머신을 절대 놓지 않는 빈 세로 통로** 를 미리 비워두고,
> 그 레이어 경계를 가로지르는 **모든 연결을 이 통로 안에서만** 달리게 한다.
> [[용어사전#채널 (channel)|채널]]은 항상 비어 있으므로 **라우팅이 실패할 수 없다** (정의상).

이것이 (롤백된) 완전탐색 전략 S-EXH 의 `FailureLeaf` 백트래킹·`n!` 형제순서 폭발을 없애는 핵심
메커니즘이며, 현재 전략 S-LAYER ([layeredWizard.ts](../../../src/autoLayout/layeredWizard.ts)) 로 구현돼 있다.
...
| **[[용어사전#채널 (channel)\|채널 (channel)]]**                   | 인접한 두 레이어 **사이**의, 머신이 없는 예약된 세로 통로.                             |
...
현재 코드의 `CHANNEL_MIN = 3` ([layeredWizard.ts](../../../src/autoLayout/layeredWizard.ts)) 이 이 계산의 *하한*이다.
...
| S-LAYER 요소 | 현재 코드 |
|---|---|
| 채널 폭 (하한 + 동적) | `CHANNEL_MIN = 3` 하한 + `channelWidthFromTracks` ([channelPlanner.ts](../../../src/autoLayout/planner/channelPlanner.ts)), 열 x 누적은 [layeredWizard.ts](../../../src/autoLayout/layeredWizard.ts) |
| 트랙 내 벨트/투입기/파이프 깔기 | `routeWithFallback` ([routeFallback.ts](../../../src/autoLayout/manualEdit/routeFallback.ts)) → `routePorts` ([manualEdit/facadeRouting.ts](../../../src/autoLayout/manualEdit/facadeRouting.ts)) / `commitRouting` ([execution/emitPath.ts](../../../src/autoLayout/execution/emitPath.ts)) |
| 라우팅 실패 처리 | 백트래킹 없음 — 실패는 `routeFailures` 카운트만(채널 보장으로 사실상 미발생). `FailureLeaf` 는 머신매칭 실패 전용 |
| 머신 좌표 결정·commit | 좌표 [layeredWizard.ts](../../../src/autoLayout/layeredWizard.ts), footprint commit `commitContainer` ([machinePlacer.ts](../../../src/autoLayout/execution/machinePlacer.ts)) |
