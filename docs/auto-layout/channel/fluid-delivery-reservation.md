...
([channel/shape.shapesConflict](../../../src/autoLayout/channel/shape.ts)):
두 도형이 같은 칸을 쓰면 충돌.
...
([run/ledger.ts](../../../src/autoLayout/run/ledger.ts) `fluidNetworksOf` · `fluidBlockedOf`).
...
([channel/policy.surfaceItemsOf](../../../src/autoLayout/channel/policy.ts)).
...
- **유체 링크 할당** — `allocateFlows` 는 유체에 여전히 불필요
...
### 8.1 `routeOneFluidDelivery` 을 미뤘다가 **2026-09-08 에 지웠다**

§4.6 은 삭제를 적었는데, 그때는 `AUTO_LAYOUT_CHANNEL_GEOMETRY` 를 끄면 아이템도 전부 탐색으로
가는 "계획 없음" 모드가 있었고 거기서 유체만 유별나게 실패시킬 이유가 없어 남겼다.
**off 모드를 지우면서 함께 지웠다.**

> **지우다가 구멍 하나를 찾았다.** *"파이프 prototype 이 없으면 실패"* 라는 판정을
> **옛 탐색 경로만** 갖고 있었다. 그대로 지웠다면 `finishFluidChain` 의 `!` 가 **이름 없는
> 파이프**를 깔았을 것이다 — 실패가 아니라 조용한 쓰레기 배치다. 그 판정을 계획 경로로
> 옮겼다(`no-pipe-entity`). **두 경로가 있으면 한쪽에만 있는 검사가 생긴다**는 사례다.
