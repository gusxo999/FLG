...
- `productOf(s)` = 출력 라인 이름 ([modulePacking.ts:338](../frontend/src/utils/autoLayout/planner/modulePacking.ts#L338)).
  라인의 `kind`(belt/pipe)를 안 본다 → 유체 출력 노드도 그대로 통과.
- `pairHopPorts` 는 이름이 같은 출력·입력 포트를 짝짓는다
  ([modulePacking.ts:205](../frontend/src/utils/autoLayout/planner/modulePacking.ts#L205)).
  유체 포트는 `linkId` 가 없어 ②번 위치-zip 으로 짝이 된다(v1 모듈당 유체 1포트 → 자명).
- 그 짝이 그대로 홉 입력에 쌓인다
  ([modulePacking.ts:521](../frontend/src/utils/autoLayout/planner/modulePacking.ts#L521)).
  `eligible` 판정은 **변(side)만** 본다 — 자식 출력이 W변, 부모 입력이 E변, 깊이 인접.
  유체 포트도 `meta.side` 를 똑같이 갖는다
  ([clusterModule.ts:1749](../frontend/src/utils/autoLayout/module/clusterModule.ts#L1749)).
- 적격이면 `DeliveryInput` 으로 장부에 들어가고, 아니면 폭만 예약
  ([modulePacking.ts:558-561](../frontend/src/utils/autoLayout/planner/modulePacking.ts#L558-L561)).

**즉 유체 상자가 W/E 변에 오면 그 홉은 계단꼴 계획을 받고 트랙을 하나 차지한다.**

그리고 **유체는 항상 W/E 변에 온다** — 선택이 아니라 강제다
([moduleWizard.ts:149](../frontend/src/utils/autoLayout/planner/moduleWizard.ts#L149)):
...
([moduleHop.ts:253](../frontend/src/utils/autoLayout/planner/moduleHop.ts#L253)):
...
([moduleHop.ts:411-415](../frontend/src/utils/autoLayout/planner/moduleHop.ts#L411-L415)).
...
| 밟힌 아이템 홉 | `plannedChainClear` 실패 → dijkstra 폴백 → **연쇄**([moduleHop.ts:286](../frontend/src/utils/autoLayout/planner/moduleHop.ts#L286) 주석의 그 연쇄) |
...
([channelGeometryPlanner.ts:180](../frontend/src/utils/autoLayout/planner/channelGeometryPlanner.ts#L180)):
두 도형이 같은 칸을 쓰면 충돌.
...
([moduleWizard.ts:417](../frontend/src/utils/autoLayout/planner/moduleWizard.ts#L417)).
...
([moduleHop.ts:514](../frontend/src/utils/autoLayout/planner/moduleHop.ts#L514)).
...
떼지 않고 그대로 이음에 쓴다([moduleHop.ts:263](../frontend/src/utils/autoLayout/planner/moduleHop.ts#L263)).
...
([channelGeometryPlanner.ts:429-442](../frontend/src/utils/autoLayout/planner/channelGeometryPlanner.ts#L429-L442)).
...
- **(가) 지하파이프 횡단 도입** — D2 를 뒤집어 pipe-to-ground 페어링까지 장부에 모델링. 정공법이나 작업이 크다.
- **(나) 채널 밖 우회** — 유체 한 줄을 채널이 아니라 모듈 위/아래(N/S 마진)로 돌린다. 장부에 네 번째 경로 종류가 생긴다.

계측 전에는 고르지 않는다 — 빈도가 0 에 가까우면 거절로 두는 게 맞다.
...
([moduleHop.ts](../frontend/src/utils/autoLayout/planner/moduleHop.ts) 의 그 거래). 밟힌 게
