...
### 함정 추가 (2026-07-19 트렁크 공유 작업에서 발견)

- **tap/direct 판정이 링크 모델과 어긋날 수 있다** — planner(insertingPlanner)는 링크를 모른 채
  옛 셈(requiredInserterCount)으로 좌석을 세고, 자식·부모가 **독립으로** tap/direct 를 판정한다.
  자식이 direct 로 떨어지면(예: 머신당 팔 6 > W면 3행) 자식은 옛 direct 포트(머신×팔), 부모는
  새 링크 포트 → **포트 모양이 어긋나 홉이 샌다.** 지금은 양쪽 다 tap 인 경우만 새 경로가
  성립. 근본 수리 = planner 좌석 셈을 링크-인지로(또는 emit 의 W/E 면 넘나들기) — 거대 출력
  N/S gap 작업과 같은 뿌리다.
