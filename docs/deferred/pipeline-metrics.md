...
> **왜 만들었나:** [.trunk-redesign §10.4](../auto-layout/module/trunk-redesign.md) — 공급 방식을
> 갈아타기 전에 **숫자로 이기는 걸 먼저 본다**.

# 파이프라인 계측기 — 같은 트리를 같은 자로 잰다

> **⚠️ 상태(2026-07-25): 도구는 삭제됐다.** `planner/pipelineMetrics.ts` 와 그 테스트를
> 지웠다 — `measurePipeline`·`formatMetrics` 를 부르는 프로덕션 코드가 0개였고 `debugApi`
> 에도 안 걸려 있어 브라우저에서 부를 방법이 없었다(자기 테스트만 호출).
>
> **문서는 남긴다** — 아래 수치는 1:1 시절의 **실측 기록**이고, 다음에 같은 비교를 할
> 때 재시작점이 된다. 다시 필요해지면 도구를 되살리되, **이번엔 debugApi 에 배선해**
> 브라우저에서 부를 수 있게 할 것.
...
[.trunk-redesign §3](../auto-layout/module/trunk-redesign.md) 이 보였듯 **단조적이지 않다**.
...
사용자가 정한 [작업 우선순위](../auto-layout/module/trunk-redesign.md#7-작업-우선순위-사용자-결정)를
...
([교차의 불가피성](../auto-layout/module/trunk-redesign.md#7-작업-우선순위-사용자-결정)의 귀결).

**`rawPorts` 가 진짜 부담이다.** 8/6/4 에서 포트 68 개 중 홉으로 짝지어진 건 10 개뿐이고
**48 개가 외부 공급/흡수로 남아** 전부 반출 대상이 된다. 이게 `exportSkips` 12 건의 출처다.

## 3b. 탭 인서팅(트렁크) 실측 — 2026-07-12, ③ 방출기 도입 후

같은 트리·같은 자.

```
tag     상태   area          채널폭  belt  ug  ins  chest  port  hop(ug)  raw  fail(hop/skip)
1/1/1   OK     408 (24×17)   [4]      65    0   29     7     11   2(0)     7   0/0
2/2/2   OK     598 (26×23)   [4]     105    0   40     7     11   2(0)     7   0/0
3/3/3   OK     754 (26×29)   [4]     141    0   51     7     11   2(0)     7   0/0
4/4/2   OK     806 (26×31)   [4]     158    0   56     7     11   2(0)     7   0/0
6/4/2   OK     806 (26×31)   [4]     174    2   64     7     11   2(1)     7   0/0
8/6/4   OK    1118 (26×43)   [4]     246    2   86     7     11   2(1)     7   0/0
3/2/5   OK     832 (26×32)   [4]     144    2   53     7     11   2(1)     7   0/0
```

### 이겼는가 — §4 의 기준으로

| 값 | 1:1 (1/1/1 → 8/6/4) | 트렁크 | 판정 |
|---|---|---|---|
| **실패**(홉/skip) | 0/0 → 1/12 | **전 구간 0/0** | ✅ 결정적 |
| **채널 폭** | 4 → **13** (머신 수에 비례) | **4 고정** | ✅ 결정적 |
| 경계 포트 | 11 → **68** | **11 고정** (= 줄 수) | ✅ |
| raw 포트 | 7 → **48** | **7 고정** | ✅ |
| 무한상자 | 7 → **50** | **7 고정** | ✅ |
| 홉 | 2 → 10 | **2 고정** | ✅ |
| 지하벨트 | 10 → **70** | **0~2** | ✅ (교차가 사라졌다) |
| 면적 | 242 → 1147 | 408 → 1118 | ⚠ 작은 count 에선 **손해** |
| belt 칸 | 28 → 242 | 65 → 246 | ⚠ 비슷하거나 약간 손해 |

**핵심:** 1:1 에서 머신 수에 비례해 폭발하던 것들(채널 폭·포트·상자·지하벨트)이 **전부 상수**가
됐다. 그게 트렁크의 정체다 — [교차의 불가피성](../auto-layout/module/trunk-redesign.md#7-작업-우선순위-사용자-결정)이
O(머신×품목) 통행량에서 왔으므로, 통행량을 O(품목)으로 접자 지하벨트가 70 → 0~2 로 증발했다.

**대가(정직하게):** count 가 작으면 트렁크가 **손해다**. 1/1/1 에서 면적 242 → 408. 머신 1대에
belt 한 줄을 까는 건 순수 낭비다(§4 의 비-단조성이 실제로 나타난 것). count=1 에서는 다이렉트가
낫다 — [insertingPlanner](../용어사전.md#insertingplanner) 가 머신 수를 보고 고르게 하는 것이
후속 후보다.
