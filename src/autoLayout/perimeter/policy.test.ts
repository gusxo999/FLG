...
import { planPerimeterExits } from "./policy";
import type { ExitContext, ExitPortInput } from "./types";
...
describe("막힌 상자 계측 — **강등이 못 세는 것**을 센다", () => {
   ...
   * (`cands` 필터) 강등 기록에 **원리적으로** 안 나온다. 2홉의 빈칸(`exit-ray.md` §4)이
   * 실제로 상자를 가두는지가 바로 그 수라, 여기서 안 세면 아무 데도 안 남는다.
   */
