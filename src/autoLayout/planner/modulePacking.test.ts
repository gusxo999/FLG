...
describe("packModuleTree", () => {
  ...
  it("반출 트랙 예약(조각 6-①) — exitPlan 항상 emit, reserve 시 bbox 마진 프레임 확장", () => {
    const off = packModuleTree(specs, config);
    const on = packModuleTree(specs, { ...config, reservePerimeterExits: true });

    // exitPlan 은 off 경로에서도 계산돼 실린다(②③ 소비용). 살아남은 상자마다 배정 1개.
    // rawPorts 가 **루트 출력까지 포함**하므로(짝 없는 포트 전부) 여기 +1 은 없다.
    expect(off.exitPlan.assignments.length).toBeGreaterThan(0);
    expect(off.exitPlan.assignments).toHaveLength(off.rawPorts.length);
    ...
    const m = on.exitPlan.marginNeeds;
  ...
  it("포트 tapAnchor(⑥B) — machine 끝점 = anchor−2·faceVector, anchor 와 겹치지 않음", () => {
    ...
    for (const res of [packModuleTree(specs, config), packModuleTree(specs, { ...config, reservePerimeterExits: true })])
      for (const pl of res.placements)
