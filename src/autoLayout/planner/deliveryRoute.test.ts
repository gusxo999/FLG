...
// 유체 납품 경로 트리(docs/auto-layout-wizard.fluid-delivery.md): 자식 gasmaker 가 petroleum-gas(유체)를
...
describe("routeDeliveryRoutes", () => {
  it("납품 경로 1개(copper-cable) 라우팅 성공, 실패 0", () => {
  ...
  it("양끝 경계 무한상자 2개를 strip 대상으로 표시", () => {
    ...
    expect(res.strippedChestIds.has(delivery.from.chest.id)).toBe(true);
    expect(res.strippedChestIds.has(delivery.to.chest.id)).toBe(true);
    expect(res.strippedChestIds.size).toBe(2);
  });

  // **양 끝이 다 벨트 모양이다** — 공급 모델 통합(2026-08-05) 이후.
  //
  // 예전엔 이 트리가 *섞인* 납품 경로였다: 부모는 입력이 많아 트렁크가 거절돼 1:1 포트
  // `[상자][인서터][머신]` 을 냈고, 자식은 탭이라 `[상자][인서터][벨트]` 를 냈다. 좌석
  // 인서터의 운명이 끝마다 갈렸고(1:1 인서터는 **머신에 재료를 넣는 유일한 물건**이라 남긴다),
  // 그래서 판정이 납품 경로 단위 플래그가 아니라 **포트 단위**여야 한다는 증거였다.
  //
  // 지금은 트렁크가 거절돼도 기계별 포트가 **같은 벨트 모양**으로 난다(링크와 같은 방출기).
  // 그래서 이 트리로는 더 이상 섞이지 않는다 — 판정이 포트 단위라는 사실은 그대로이고
  ...
  it("트렁크든 기계별 포트든 — 양 끝 좌석이 belt 로 메워진다", () => {
    ...
    const seatOf = (p: typeof delivery.from) => {
      const fv = faceVector(p.face);
      return `${p.anchor.x - fv.x},${p.anchor.y - fv.y}`;
    };
    const fromSeat = seatOf(delivery.from);
    const toSeat = seatOf(delivery.to);
    ...
    // 기계별 포트 끝 — 이쪽도 벨트 모양이라 같은 운명이다(belt→belt 피더라 떼고 메운다).
    // 머신에 재료를 넣는 인서터는 **좌석 줄(d1)의 탭**이고 그건 손대지 않는다.
    expect(res.strippedCellKeys.has(toSeat), `포트 인서터가 안 떨어짐 @${toSeat}`).toBe(true);
    expect(
      res.cells.find((c) => `${c.x},${c.y}` === toSeat)?.cell.entityType,
      `포트 좌석이 belt 로 안 메워짐 @${toSeat}`,
    ).toBe(EntityType.Belt);
  });

  it("유체 납품 경로 — 자식 유체 출력 → 부모 유체 입력을 파이프로 잇는다", () => {
    const pack = packModuleTree(fluidSpecs, packConfig);
    // packModuleTree 가 이름 기반으로 유체 포트도 짝짓는다 → 유체 DeliverySpec 이 만들어진다.
    const fluidDelivery = pack.deliveries.find((h) => h.item === "petroleum-gas");
    expect(fluidDelivery, "유체 납품 경로 쌍이 안 만들어짐").toBeDefined();
    expect(fluidDelivery!.from.chest.kind).toBe("infinity-pipe"); // 유체 포트

    const res = routeDeliveryRoutes(pack, fluidDeliveryConfig);
    expect(res.failures).toBe(0);

    // 납품 경로 경로는 **파이프**로 깔린다(벨트가 아니라) — pipe-to-pipe.
    ...
    expect(
      ...
      "납품 경로 경로에 파이프 아닌 셀이 있다",
    ).toBe(true);

    // 양끝 무한파이프를 strip 대상으로.
    expect(res.strippedChestIds.has(fluidDelivery!.from.chest.id)).toBe(true);
  ...
  it("strip 후 납품 경로 belt 와 모듈 셀이 겹치지 않음 (좌표 고유)", () => {
    const pack = packModuleTree(specs, packConfig);
    const res = routeDeliveryRoutes(pack, deliveryConfig);
    ...
    for (const c of res.cells) {
      const k = `${c.x},${c.y}`;
      expect(occ.has(k), `납품 경로 belt ${k} 가 모듈 셀과 충돌`).toBe(false);
      occ.add(k);
    }
  });

  it("납품 경로 belt 는 전부 직교 이웃을 향함 (지상 연속 흐름)", () => {
    const pack = packModuleTree(specs, packConfig);
    const res = routeDeliveryRoutes(pack, deliveryConfig);
    const set = new Set(res.cells.map((c) => `${c.x},${c.y}`));
    // 각 belt 방향이 가리키는 다음 셀은 또 다른 납품 경로 belt 이거나 trunkStart(기존 모듈 belt).
    ...
    for (const c of res.cells) {
      ...
      const nk = `${nx},${ny}`;
      const intoDelivery = set.has(nk);
      const intoModule = occ.has(nk);
      expect(intoDelivery || intoModule, `belt (${c.x},${c.y}) dir ${c.cell.direction} 가 허공을 가리킴`).toBe(true);
      if (!intoDelivery && intoModule) endpointsHittingTrunk += 1;
    }
    // 정확히 1곳: 부모 입력 trunkStart 로 들어가는 seat_to belt.
    expect(endpointsHittingTrunk).toBeGreaterThanOrEqual(1);
  });

  it("[불변식] 지하벨트 활성·멀티납품 경로에서도 belt 체인은 텔레포트하지 않는다", () => {
    ...
    const res = routeDeliveryRoutes(pack, ugConfig);
    expect(res.failures).toBe(0);
    for (const route of res.routes) {
      ...
      for (let i = 0; i + 1 < route.cells.length; i++) {
        ...
        expect(isJumpPair, `납품 경로 ${route.item}: (${a.x},${a.y})→(${b.x},${b.y}) 텔레포트(거리 ${md})`).toBe(true);
      }
    }
  });

  it("[불변식] 모든 납품 경로 belt 는 하류가 belt/인서터/머신 — 허공·orphan 없음", () => {
    const pack = packModuleTree(emSpecs, packConfig);
    const res = routeDeliveryRoutes(pack, ugConfig);
    const DIRVEC: Record<number, { x: number; y: number }> = {
      0: { x: 0, y: -1 }, 4: { x: 1, y: 0 }, 8: { x: 0, y: 1 }, 12: { x: -1, y: 0 },
    };
    const deliverySet = new Set(res.cells.map((c) => `${c.x},${c.y}`));
    const occ = occupancyOf(pack);
    for (const k of res.strippedCellKeys) occ.delete(k);
    for (const c of res.cells) {
      ...
      expect(deliverySet.has(nk) || occ.has(nk), `납품 경로 belt (${c.x},${c.y}) 하류가 허공`).toBe(true);
    }
  });

  it("seat_from 은 자식 출력 trunkStart 와 인접 (기하 유도 검증)", () => {
    const pack = packModuleTree(specs, packConfig);
    routeDeliveryRoutes(pack, deliveryConfig); // pack 미변형(순수) — 던지지 않는지만 본다
    const delivery = pack.deliveries[0];
    const fv = faceVector(delivery.from.face);
    const trunkStart = { x: delivery.from.anchor.x - 2 * fv.x, y: delivery.from.anchor.y - 2 * fv.y };
  ...
  it("결정적", () => {
    const a = routeDeliveryRoutes(packModuleTree(specs, packConfig), deliveryConfig);
    const b = routeDeliveryRoutes(packModuleTree(specs, packConfig), deliveryConfig);
    const sig = (r: ReturnType<typeof routeDeliveryRoutes>) =>
  ...
  function injectWall(pack: PackResult): { wallX: number } {
    const delivery = pack.deliveries[0];
    const wallX = Math.round((delivery.from.anchor.x + delivery.to.anchor.x) / 2);
  ...
  it("지하벨트 게이트 오프(미선택): 막힌 벽을 **뚫지 않는다** — 지하 셀 0, 대신 실패", () => {
    const pack = packModuleTree(specs, packConfig);
    injectWall(pack);
    const res = routeDeliveryRoutes(pack, deliveryConfig); // underground 미지정

    // 이 벽은 배치 bbox 를 세로로 가로막는다. 납품 경로 dijkstra 는 bbox 안에 갇혀 있으므로
    // (routeOneDelivery 의 bounds — 없으면 무한 격자를 탐색하다 OOM), **지상 우회로가 없다.**
