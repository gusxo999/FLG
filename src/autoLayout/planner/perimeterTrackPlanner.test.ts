...
import { planPerimeterTracks, type TrackContext, type TrackPortInput } from "./perimeterTrackPlanner";
...
const p = (
  x: Omit<TrackPortInput, "wayOuts"> & { wayOuts?: PortFace[] },
): TrackPortInput => ({ ...x, wayOuts: x.wayOuts ?? ALL });

// 3 열(depth 0..2), 각 열 모듈 1개, 세로 밴드 [0,9].
const ctx3 = (): TrackContext => ({
  globalY: { min: 0, max: 9 },
  maxDepth: 2,
...
describe("planPerimeterTracks", () => {
  it("N/S 변 포트 = 자기 열 직진(self), 채널 소비 0, N/S 마진 예약", () => {
    ...
    const plan = planPerimeterTracks(ports, ctx3());
  ...
  it("자기 열 위 형제에 막힌 N 변 = 인접 채널로 우회", () => {
    ...
    const ports: TrackPortInput[] = [p({ id: "p", role: "output", depth: 1, side: "N", anchorY: 6 })];
    const plan = planPerimeterTracks(ports, ctx);
    expect(plan.assignments[0].host).toEqual({ kind: "channel", depth: 1 });
    // sib0(위) 의 N 변 포트 → 막힘 없음 → self.
    const top: TrackPortInput[] = [p({ id: "q", role: "output", depth: 1, side: "N", anchorY: 0 })];
    expect(planPerimeterTracks(top, ctx).assignments[0].host).toEqual({ kind: "self" });
  });
  ...
  it("결정적 — id 순 안정", () => {
    ...
    const s = (ps: TrackPortInput[]) => JSON.stringify(planPerimeterTracks(ps, ctx3()).assignments);
    expect(s(ports)).toBe(s([...ports].reverse()));
  });
});

describe("planPerimeterTracks — moduleWayOuts 제약", () => {
      ...
      for (const side of ["N", "S", "W", "E"] as const)
        for (const depth of [0, 1, 2])
          for (const anchorY of [1, 5, 8]) {
            const plan = planPerimeterTracks(
