...
| `link/` | 두 모듈의 **식별자**를 안다 | 타입 `types`(DeliveryConfig · DeliveryResult) · `allocateFlows` · `edgeLinks` · 정책 `policy`(edgeLinksOf · 납품 사다리 chooseRoute) · 셈 `arith`(pairDeliveries) · 장부 `ledger`(plannedChainClear · recordRoute) · 도형 `shape`(buildPlannedChain) · 찍기 `emit`(finishChain) — 뼈대는 평면의 `deliveryRoute` | `docs/auto-layout/link/` |
...
| `perimeter/` | **전역 외곽**을 안다 | 타입 `types`(ExitPortInput · ExitContext · PerimeterExitPlan) · 도형 `shape`(출구 자격 — directOptionOf · channelEntryOf) · `wayOuts` · `exits` · 직진 장부 `directRay` — 선호와 강등은 평면의 `perimeterExitPlanner` | `docs/auto-layout/perimeter/` |
