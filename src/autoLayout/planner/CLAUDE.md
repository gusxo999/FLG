...
| `channel/` | **여러 연결이 나눠 쓰는 자원**을 안다 | 타입 `types`(DeliveryInput · ExportInput · ChannelGeometryPlan) · 장부 `ledger`(rowChannelsOf · planChannels) · 도형 `shape`(materializeChannelGeometry) — 평면의 `channel*Planner` 는 계획 3 이 옮긴다 | `docs/auto-layout/channel/` |
