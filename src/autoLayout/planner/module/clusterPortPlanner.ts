...
export function insertingPlanner(
  ...
  // **구간 쪼개기는 여기 없다**(2026-09-02 삭제 — 옛 `splitIntervals`, `c = 2` 까지).
  //
  // 부분 트렁크를 탭 경로에 넣으려던 시도였는데, 그 산출(`tapPlan.lines`)을 **아무도 안
  // 읽는다** — `planModulePorts` 가 `rest: { ok: true, lines: [] }` 로 버리고, 기하는
  // `externalLineGroups` → [tryLinkFace] 가 만든다. 같은 일을 **살아 있는 경로에서**
  // `bundle`(= `g`)이 하고, `g` 를 정하는 것은 이제 레인 예산이다
  // (`planner/module/laneBudget.ts` · `trunk-assignment.md` §4.2).
