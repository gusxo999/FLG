 ...
 * ```
 * 막힌 칸이 **점**  → 쪼갬      1단이 푼다
 * 막힌 칸이 **구간** → 좌석막힘  2·3단의 몫 (이 칸의 실패가 아니다)
 * ```
 */
...
describe("rungOfLine — 이 실패는 어느 칸의 몫인가", () => {
  it("막힌 칸이 **점**이면 `쪼갬` — 1단이 맡는다 (실측 d3: 못 90·180)", () => {
    expect(rungOfLine([at({ seatRows, blockedRows: [90, 180] })])).toBe("쪼갬");
  });

  it("막힌 칸이 **구간**이면 `좌석막힘` — 2·3단의 몫이다 (실측 d2: 남의 벨트)", () => {
    expect(rungOfLine([at({ laneDepth: 2, seatRows, blockedRows: belt })])).toBe("좌석막힘");
  });

  it("좌석 예산이 모자라면 `좌석부족` — 자를 것이 있고 없고의 문제가 아니다", () => {
    expect(rungOfLine([at({ seats: { need: 4, budget: 3 } })])).toBe("좌석부족");
  });

  it("벨트는 지나가는데 포트 칸이 막히면 `포트막힘` — 1단의 몫이나 아직 안 만들었다", () => {
    expect(rungOfLine([at({ blockedPort: [[7, 4]] })])).toBe("포트막힘");
  });

  it("후보가 갈리면 **가장 싼 칸**이 맡는다 — 하나라도 쪼개지면 1단의 일이다", () => {
    expect(rungOfLine([
      at({ laneDepth: 2, seatRows, blockedRows: belt }),   // 좌석막힘
      at({ laneDepth: 3, seatRows, blockedRows: [90] }),   // 쪼갬
    ])).toBe("쪼갬");
  });
...
describe("summarizeRungs — 모듈 한 개의 경계를 한 문장으로", () => {
  it("칸별로 세고 **싼 칸부터** 적는다", () => {
    ...
    expect(summarizeRungs(m)).toBe("쪼갬 1 · 좌석막힘 2");
  });
