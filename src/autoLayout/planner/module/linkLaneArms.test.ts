 ...
 * A 는 아무것도 안 지키고, 포트 행을 비껴가는 배열을 함께 안 재면 B 는 *"깊은 레인을 아예
 * 안 준다"* 와 구별되지 않는다.
 *
...
const link = (from: number[], rate: number): Link => ({
  item: "gear",
  // `from` 의 수는 reach 1 기준 팔 수다. **진짜 출처는 `carries`** 이고, 배정이 레인마다
...
function lanesOf(mod: GeneratedModule) {
...
describe("결함 A — 레인이 팔 종류를 바꾸면 팔 개수도 바뀐다", () => {
  it("뒤 줄이 실제로 둘째 레인(d3)에 앉았다 — 아니면 아래가 아무것도 안 지킨다", () => {
    expect(lanesOf(pushedDeep)[1].depth).toBe(3);
  });

  it("**d3 에 앉은 줄의 팔은 긴팔로 센 값**이다 — 같은 rate 가 팔 2개가 된다", () => {
    expect(lanesOf(pushedDeep)[1].cells).toBe(2);
  });

  it("**대조군** — 같은 줄이 혼자 앉으면 d2 이고 팔은 1개다", () => {
    // 관통 줄을 빼면 d2 가 비어 있어 같은 줄이 짧은팔로 앉는다. 레인만 달라졌는데
    // 팔 수가 갈린다는 것이 §16 의 전부다.
    const alone = generateModule(base([link([0], RATE)]));
    expect(lanesOf(alone)[0]).toEqual({ depth: 2, cells: 1 });
  });
...
describe("결함 B — 포트 칸도 표의 칸이다", () => {
  ...
  it("앞 줄의 **포트 행**을 삼키는 관통 줄은 그 레인을 못 받는다", () => {
