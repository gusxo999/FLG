/**
 * **레인 예산** — 줄마다 `g`(묶음 크기)를 정한다. 자리를 앉혀 보기 **전에**, 수만으로.
 *
 ...
 *
 * ```
 * g = 1   머신 한 대의 `a_j(m)` 행만        → 여러 품목이 한 레인을 **나눠 쓴다**
 * g > 1   첫 머신 첫 행 … 끝 머신 끝 행     → **사이 행까지** 먹어 레인을 독점한다
 * ```
 *
 * 그래서 상한에 `g` 가 들어간다 — 처리량이 준 `g_max`([bundleCap])를 **레인이 더 깎는다**:
 *
 * ```
 ...
 * 레인도 같은 이유로 같은 낟알이다. 면마다 세면 **넘어갈 수 있는 자리를 없다고 세어**
 * 관통을 헛되이 깎는다(2026-09-01 실측: electric-motor 4줄이 E 두 레인만 보고 둘을 내렸다 —
 * 실제로는 W 의 남은 레인으로 넘어가 넷 다 관통이 선다).
 *
 * 예약이 **면 수**인 것은 `g=1` 줄이 여러 면에 흩어질 수 있어서다 — 좌석이 넘치면
 * 반대 면으로 밀리고, 그 면에도 공용 레인이 하나 있어야 한다.
 *
 ...
 * > 구간을 잠수해 지나가므로 한 레인이 담는 줄이 늘고 **공짜의 범위가 넓어진다.**
...
/** 그 면에 **이미 앉은** 것이 먹은 레인 — 링크(내부 간선)가 먼저 자리를 잡기 때문이다. */
export interface TakenLanes {
  /** 머신을 걸치는 그룹 수 — 하나가 레인 하나를 독점한다. */
  spanning: number;
  /** 머신 하나짜리 그룹이 있나 — 있으면 그것들이 레인 하나를 함께 쓴다. */
  direct: boolean;
}
...
export interface LaneBudgetInput {
  ...
  /** 그 면의 레인 수 `R_f` — [laneDepthsOf] 가 낸다(유체 면은 지하파이프 사거리로 깎인 값). */
  readonly lanesOf: (face: BudgetFace) => number;
  /** 링크가 먼저 먹은 레인. 없으면 빈 면으로 본다. */
  readonly taken?: (face: BudgetFace) => TakenLanes;
}
 ...
 */
export function planBundles(input: LaneBudgetInput): Map<string, number> {
  ...
  const faces = (["W", "E"] as const).filter((f) => input.lanesOf(f) > 0);
  const base = faces.map((f) => input.taken?.(f) ?? { spanning: 0, direct: false });
  /** 링크가 이미 독점한 레인을 뺀 **남은 레인** — 관통과 공용 레인이 여기서 나눠 갖는다. */
  const lanes =
    faces.reduce((n, f) => n + input.lanesOf(f), 0) - base.reduce((n, b) => n + b.spanning, 0);
  const takenDirect = base.some((b) => b.direct);
  /** 이 줄이 레인을 **독점**하나 — 머신을 걸치면 사이 행까지 먹는다(§4.1). */
  const wide = (l: IoLine): boolean => (g.get(keyOf(l)) ?? 1) > 1;

  // **우선순위가 낮은 줄부터 내린다** — 뒤에서 앞으로. 내릴 때마다 관통이 하나 줄고,
  // 첫 번째로 내리는 순간 공용 레인 예약이 켜지므로 예산이 **단조 감소**한다 → 반드시 끝난다.
  for (let i = input.lines.length - 1; i >= 0; i--) {
    const narrow = takenDirect || input.lines.some((l) => !wide(l));
    // `g=1` 줄이 있으면 **면마다** 공용 레인 하나를 예약한다. 그 줄들은 좌석이 넘치면
    // 반대 면으로 밀리는데(그게 `g=1` 의 존재 이유다), 밀려간 면의 레인을 관통이 이미
    // 독점하고 있으면 **갈 곳을 잃는다**(2026-09-01 실측: 일곱 줄 스펙에서 포트가 사라졌다).
    const budget = lanes - (narrow ? faces.length : 0);
