...
import { planBundles, type DepthBudgetInput } from "./depth";
...
/** `depthsOf` 를 면마다 같은 수로 — `R` 은 서로 다른 reach 개수라 두 면이 대개 같다. */
const run = (
  lines: readonly IoLine[],
  R: number,
  extra: Partial<DepthBudgetInput> = {},
): Map<string, number> =>
  planBundles({
    lines,
    count: 4,
    belts: BELT,
    // 수요를 안 주면 `g_max = N` — 옛 탭 동작이고, 깊이 예산이 거기서부터 깎는다.
    depthsOf: () => R,
    ...extra,
  });
...
describe("깊이 예산 — 관통은 공짜일 때만 산다", () => {
  it("`R=1` · 입력1+출력1 → **둘 다 관통** (kr-sand 꼴)", () => {
    // 깊이 2개(W1+E1)에 줄 2개. 아무도 `g=1` 이 아니므로 공용 깊이를 예약할 것도 없다.
  ...
  it("`R=1` · 줄 셋 → **전부 다이렉트** — 하나만 내려도 예약이 켜져 나머지가 못 산다", () => {
    // 관통 3 > 깊이 2 → 하나 내린다. 그 순간 `g=1` 줄이 생겨 면마다 공용 깊이를 예약하므로
  ...
  it("`R=2` · 줄 넷 → **전부 관통** (electric-motor 꼴)", () => {
    // 깊이 4개(W2+E2)에 줄 4개가 정확히 들어간다. 면마다 세면 E 의 두 깊이만 보고 둘을
  ...
  it("내리는 것은 **우선순위가 낮은 줄부터** — 출력이 마지막까지 관통을 지킨다", () => {
    // 깊이 4 · `heavy` 가 처리량 때문에 이미 `g=1` → 예약이 처음부터 켜져 예산은 `4−2 = 2`.
  ...
  it("링크가 먹은 깊이는 **빼고 센다** — 계획이 두 번 세지 않는다", () => {
    const free = run([outL("out"), inL("a")], 2);
    const taken = run([outL("out"), inL("a")], 2, {
      // W·E 각각 관통 링크 둘씩 = 깊이 4개를 이미 다 먹었다.
  ...
  it("깊이가 0인 면은 **면 수에서도 빠진다** — 없는 자리를 예약하지 않는다", () => {
    // W 만 깊이 2, E 는 0(유체가 다 먹었다). 예약은 한 면 몫이라 예산 `2 − 1 = 1`.
