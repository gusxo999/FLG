...
/**
 * **레인 공유 짝짓기** — 줄 **둘**을 한 물리 벨트의 좌/우 레인에 하나씩 싣는다
 * ([belt-lane-semantics](../../../docs/factorio/belt-lane-semantics.md)).
 *
 * ## 무엇을 되찾나
 * 인서터는 먼 레인 하나에만 떨구므로 **줄 하나는 벨트의 절반만 쓴다**. 그래서 45/s 수요는
 * 줄 **둘**이 된다([determineBeltCount]). 그 둘을 합류시키면 벨트가 **하나**로 돌아온다 —
 * **처리량은 안 늘고 물리 벨트 수가 준다**(벨트 칸 · 채널 트랙 · 면 깊이 · 포트).
 *
 * 후보를 **고르는 것은 호출자**다 — 여기 들어오는 줄들은 이미 *"같은 물리 벨트에 실려도
 * 되는 것들"* 이라야 한다. 이 함수는 그 안에서 **양만 보고** 짝을 짓는다. 그래야 `module/`
 * 이 형제 모듈을 모른다는 축 2 판정을 지킨다.
 *
 * ## 자격 셋
 * ```
 ...
 * ```
 *
 * 마지막이 핵심이다 — 최적화 판단이 아니라 **넘으면 게임에서 안 흐른다**.
 *
 * > **품목이 같아도 된다**(2026-09-03 정정). 예전엔 *"같은 품목 둘은 합칠 이유가 없다 —
 * > 한 줄에 그냥 더 실으면 된다"* 고 적고 막았는데, **더 실을 수가 없다**: 레인 하나가
 * > 이미 다 찼기 때문에 줄이 둘이 된 것이다. 오히려 **같은 품목이 주력**이다 — 집는 팔이
 * > 뭘 집든 같은 품목이라 **필터가 필요 없다.**
 *
 * ## 결정적이다
...
export function shareLanes(
  ...
  for (let i = 0; i < open.length; i++) {
    ...
    for (let j = i + 1; j < open.length; j++) {
      const b = open[j];
      if (taken.has(b) || b.beltEntityName !== a.beltEntityName) continue;
