 ...
 * 예전엔 planPerimeterExits 가 `meta.side` 만 보고 배정해서, 코너 어깨 상자처럼 그 방향이
...
describe("예약 불변식 — 탐색 없이 방출 가능", () => {
  for (const [c0, c1, c2] of COUNTS) {
    it(`${c0}/${c1}/${c2} — 예약(hint) 재생만으로 모든 상자가 방출된다`, () => {
      ...
      const asg = new Map(pack.exitPlan.assignments.map((a) => [a.id, a]));
      ...
      for (const pl of pack.placements)
        for (const p of [...pl.module.inputPorts, ...pl.module.outputPorts]) {
          ...
          const r = routePortToPerimeter({
            ...
            perimeter,
            obstacles: occ,
            hint: { exitEdge: a!.exitEdge, exitMode: a!.exitMode, trackX: a!.trackX },
          });
  ...
  it("안 쓸 출구를 위해 채널을 넓히지 않는다 — 예약된 채널 트랙은 전부 실제로 쓰인다", () => {
    ...
    for (const a of pack.exitPlan.assignments) {
      if (a.exitMode.kind !== "channel") continue;
