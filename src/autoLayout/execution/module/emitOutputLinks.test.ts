...
describe("링크 방출은 tap/direct 판정과 무관하다", () => {
  ...
  // (여기 있던 *"모듈은 direct 로 떨어진다"* 는 **라벨만 재는 테스트**였다 — count=1 이라
  //  `g = 1` 과 `g = N` 이 같은 기하라 관측되는 차이가 없었다. 그 라벨이 사라졌고,
...
// **[[ParallelBelt]] — 막힌 면(머신 사이).** 트인 면(W/E)에서는 벨트가 자기 좌석 행에서 곧장
// 밖으로 나가 서로 만날 일이 없다. 막힌 면은 나가는 쪽(서쪽)이 이웃 머신에 가려 **면과 평행**
// 으로 돌아 나가야 하고, 좌석 줄이 하나뿐이라 벨트가 전부 같은 줄을 쓰려 든다 — **그러면
// 합쳐진다.** 합류하면 뒤엣것이 막혀 장부가 거짓이 되므로, n 번째 그룹은 **한 칸 더 깊은 줄로
// 내려가** 자기 줄로 나간다. 내려가는 건 벨트→벨트라 팔 길이와 무관하다(팔은 d2 까지만).
describe("ParallelBelt(막힌 면) — 한 면에 벨트 여러 줄, 합류 없음", () => {
  ...
  it("gap 두 그룹의 포트가 서로 다른 줄에 선다 — 합류하지 않는다", () => {
  ...
  it("gap 폭이 자기 줄 깊이에서 나온다 — 같은 면은 더하지 않고 가장 깊은 것 하나", () => {
    const [m0, m1] = mod.machines;
    // 좌석 1 + 수집 1 + 둘째 줄 1 = 3. 두 그룹 깊이를 더했다면 5가 됐을 것.
