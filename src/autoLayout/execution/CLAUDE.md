...
`unroutedLines` 에 넣는 자리는 두 가지뿐이다(2026-09-14 판정 — work-kinds §7):

```
받아 적기   배정이 없는 그룹 — 계획이 이미 답했다(좌석 없음 · 사유는 계획의 depthShortages)
안전망      칸이 막혀 대안 없이 줄을 낸다 — "구성상 발생 안 함". 발동하면 장부가 그 칸을 몰랐다는 뜻이다
            (오늘 gap 포트 칸에서 발동한다 — D8)
```
