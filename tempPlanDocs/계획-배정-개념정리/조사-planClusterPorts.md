...
④  ~~"면당 R줄" 은 **낡은 상한**이다(J1) — 지우면 될 배치가 늘어난다~~
    **2026-09-01 정정 — 반만 맞았다.** 나눠 쓰는 것은 `g=1` 줄뿐이고, `g>1` 줄은
    레인을 하나씩 **독점한다**(`trunk-assignment.md` §4.1). 그래서 상한은 낡은 게
    아니라 **`g` 를 안 보는 것**이 틀렸다: `(g>1 줄 수) + (g=1 줄 있으면 1) ≤ R_f`.
    → 지우려면 누군가 `g` 를 정해 줘야 하고, 그건 ① 다음이다(아래 순서)
```

### ② 는 대체가 있다

지도 B 의 이름표(`rungOfLine` — `좌석부족`·`좌석막힘`·`구간막힘`·`포트막힘`)를 나머지 줄까지
넓히면 된다. `depthShortages` 의 키를 `id ?? "역할:품목"` 으로 넓히는 일이다(J3).
...
> ④ 가 없어지면 *"이 줄은 `g=1`"* 을 정하는 사람이 사라지고, 넘치지 않은 줄이 `g=N` 을
> 되찾아 **레인을 독점**한다 → 나머지 줄이 자리를 잃는다(battery 실측).
> 기록은 `tempPlanDocs/부분-트렁크/부분-트렁크.md` §3.
