...
**[[링크-약속량]] 이 먼저다.** Step 2 는 폭을 넓히는 변경이고, 그 대가는 채널을 지나는
**경로 수에 비례**한다. 링크 접기(그 계획 Step 2)가 glass 100/s 의 경로를 54 → 5 로
줄이므로, 순서를 바꾸면 멀쩡한 트리에서 폭이 폭발한 것처럼 보인다.
...
- **링크·다이렉트 폴백을 안 건드린다.** 경로 **수**를 줄이는 일은 [[링크-약속량]] 소관이다.
