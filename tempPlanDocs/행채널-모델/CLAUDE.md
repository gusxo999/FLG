...
>
> **다음은 실물 재측정이다** — 브라우저에서 `delivery-dijkstra-fallback` 이 실제로
> 주는지. 그 뒤에 납품 라우터에 남은 것이 있는지 판단한다(그때까지 라우터는 안 건드린다).
>
> **정정 하나:** 갱신안 §1.4 가 *"이미 띠 밖으로 밀어내고 있다"* 고 적었는데 **틀렸다** —
> `높이 3 → 수요 4` 는 `trackCount + 2` 의 여유에서 온 값이고 트랙 2개는 3칸에 들어간다.
> 넘치려면 한 띠에 트랙 4개 이상이라야 하고, 아직 관측되지 않았다.
