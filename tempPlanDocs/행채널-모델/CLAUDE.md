...
>
> **Step 6 은 착수 근거가 사라졌다**(2026-09-04) — `perimeter-skip` 은 행 채널이 아니라
> **레인 합류의 고아 상자**였고, 그쪽에서 고치자 **0건**이 됐다. 이 계획의 실측 경고는
> 이제 **하나도 없다**. Step 6 은 *"진짜 트랙 경쟁이 관측되면"* 으로 조건이 바뀐다.
>
> **납품 라우터는 끝까지 안 건드렸다** — `dijkstra 0 · 예약침범 0 · 실패 0` 이라
> 폴백 경로가 한 번도 안 돈다. 안 도는 코드는 고쳐도 고쳤는지 알 수 없다.
>
> **정정 둘:**
