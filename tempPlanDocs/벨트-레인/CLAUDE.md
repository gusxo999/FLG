...
>
> **계측은 마지막 관문에서 센다** — `짝` 이 아니라 **`합쳐짐`** 이 아낀 벨트다(㉡-5).
> 되돌림은 **도형/좌석**으로 갈려 있어 처방이 바로 읽힌다.
>
> **아직 기본 꺼짐이다.** 남은 것은 경고 **하나**이고 **주인이 여기다**(2026-09-04 확인):
>
> ```
> perimeter-skip 4건 = 짝의 **둘째 상자가 고아**가 된다
>   planTracks    짝지어졌으니 반출 대상에서 뺀다
>   deliveryRoute 납품이 안 돌았으니(합류가 하나로 접었다) strip 도 안 한다
>   ⇒ 아무도 안 맡는다. 넘어간 넷이 전부 둘째 논리 포트다(`…-1` · `…-5`)
> ```
>
> **고칠 자리:** 둘째 상자는 갈 곳이 없는 게 아니라 **갈 필요가 없다**(첫 줄과 같은 칸).
> `strippedChestIds` 에 넣으면 끝난다. 행 채널 Step 6 이 아니라 **여기 몫**이다.
>
> `delivery-dijkstra-fallback` 은 **2026-09-04 에 사라졌다**(행 채널 Step 3·4:
> `planned 3 / dijkstra 2` → `planned 5 / dijkstra 0`).
>
> **`flg.check` 위반이 OFF 4 · ON 6** — 합류가 `[R-도달]`(포트와 머신 사이가 비었다)을
> 둘 더 만든다. 같은 뿌리(둘째 줄의 뒷정리)일 가능성이 크니 위와 함께 본다.
>
> **그 둘의 주인은 여기가 아니다**(2026-09-04 조사). 기둥 끝 포트가 쓰는 **행 채널**에
> 계획이 없어서 생긴다 — 띠가 수요보다 얇고, 띠 장부가 세로 진입을 안 본다.
> → `tempPlanDocs/행채널-모델/` §1.4. 그쪽 Step 3·4 가 끝난 뒤 **다시 재고** 켤지 정한다.
>
> **그래서 「합류 포트를 기둥 끝에서 푼다」는 안은 보류다** — 행 채널이 고장 난 것을
> 우회하는 코드가 되기 쉽다(2026-09-04 판단 철회).
>
