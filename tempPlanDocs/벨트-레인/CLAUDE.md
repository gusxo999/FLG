> 상태: **진행 중** — ㉠ ✅ · ㉡ **전부 ✅**(짝짓기·부모 면·포트·출구 합류·포트 자리 기준).
>
> **자식 쪽 출구 합류가 섰다**(2026-09-04). 이끄는 줄이 **기둥 밖 한 행**까지 달려 거기서
> 꺾고, 따르는 줄이 깊이 +2 를 타고 와 옆구리를 친다. 배정 순서는 **유체 → 합류 쌍 →
> 관통 → 구간**(계획서 ㉡-4).
>
> **실측: 켜면 에러 0 · 납품 7→6 · 셀 522→532.** 남은 둘은 경고다 —
> `delivery-dijkstra-fallback` 2건(포트가 남의 납품 예약과 겹쳐 탐색으로 돌았다) ·
> `perimeter-skip` 1건(상자가 트랙을 못 받았다). **저하지 실패가 아니다.**
>
> **아직 기본 꺼짐이다** — 그 경고 둘을 지우고 *"짝 하나가 물리 벨트 몇 개를 아꼈나"* 를
> 셀 수 있게 계측을 고치는 것이 다음이다.
>
