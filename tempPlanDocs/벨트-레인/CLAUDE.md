...
>
> **Step 1(실측)이 끝나기 전에는 Step 3 이후가 확정이 아니다.** 레인을 채우는 기하가
> 게임 물리에 달려 있고, 그 물리 중 **둘을 아직 모른다**(계획서 §2 의 ④·⑤).
...
- **출력(collect) 쪽 레인** — 기하가 불가능하다고 답한다(계획서 §4). 사이드로드가
  생겨도 그건 모듈 밖 이야기라 이 폴더 밖이다.
