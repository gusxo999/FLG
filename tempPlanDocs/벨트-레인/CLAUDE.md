...
>
> **`AUTO_LAYOUT_LANE_MERGE` 를 켜면 합류가 실제로 선다** — 부모 면이 한 벨트, 논리 포트
> 둘이 한 칸, 채널이 `staircase` + `mergeTail` 로 갈려 **탐색 폴백 0 · 실패 0**.
> 기본은 꺼짐(실물 트리 검증 전).
>
> **다음은 ㉢** — 검사가 레인을 안다(`beltFlow` 가 칸마다 유입 개수를 세고 포화를 레인별로).
>
