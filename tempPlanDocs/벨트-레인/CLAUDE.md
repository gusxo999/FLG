> 상태: **진행 중** — ㉠ ✅ · **㉡ 전부 ✅**(짝짓기·부모 면·포트·채널 합류, 2026-09-03).
>
> **켠 배치는 아직 완성이 아니다** — 부모 쪽(한 벨트·한 칸·납품 하나)은 섰는데
> **자식 쪽 출구 합류가 없어 둘째 벨트가 갈 곳이 없다.** 그래서 기본 꺼짐이다.
>
> **다음은 그 출구 합류 하나** — `emitOutputLinks` 가 기둥 끝 바깥 한 행에 합류 칸을 놓고
> 포트를 하나만 낸다(도형은 계획서 ㉡-3). 그게 끝나면 켤 수 있다.
>
> **축이 재편됐다**(계획서 §7): ㉠ 진실을 말하게 한다 → ㉡ 합류로 절반을 되찾는다 →
> ㉢ 하류. **㉡ v1 은 같은 품목**이라 필터가 필요 없다.
>
> 낱말은 되찾았고(`clusterBeltDepth` · `track` · 레인=게임 전용), 용량(`laneThroughput`)과
> 공유 자료(`Link.sharedLineId` · `shareLanes`)가 섰고, 후보 짝짓기가 **실물 픽스처에서
> 실제로 발동한다**(`flg.report()` 의 `레인공유`).
>
> **배치는 아직 하나도 안 바뀐다** — `sharedLineId` 를 읽는 방출기가 없다.
>
> **다음(4c)에서 막혀 있다 — 그런데 물음이 바뀌었다.** 포트(*"한 셀에 논리 포트 둘"*)는
> 조사해 보니 싸다(소비처가 거의 전부 `chest.id` 로 조회한다). 진짜 관문은 한 단계 앞
> — **배정이 공유를 모른다**: `tryLinkFace` 가 줄마다 자기 면·깊이·구간을 잡아서, 물리
> 줄 하나가 둘의 **합집합 구간**을 덮을 자리가 계획에 없다. 안 둘(㉮ 배정을 공유 인지로 /
> ㉯ 우연히 같은 자리일 때만)은 계획서 Step 4c.
