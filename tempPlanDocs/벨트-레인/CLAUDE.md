> 상태: **진행 중** — ㉠ ✅ · ㉡ **전부 ✅** · **경고 0 · `flg.check` 통과**(2026-09-04).
>
> **켠 쪽이 모든 잣대에서 낫거나 같다:**
>
> ```
>            셀    납품                    이슈   flg.check
>   합류 OFF  522   planned 7 / dijkstra 0   0     통과
>   합류 ON   520   planned 5 / dijkstra 0   0     통과
...
>
> **남은 결정 하나 — 기본값.** 아직 기본 꺼짐이다. 실측에서 켠 쪽이 더 낫지만
> **픽스처 하나**(electronic-circuit 10/s · 모듈 4)만 재 봤다. 켤지는 사장님 판단이다.
>
> **남은 미완성 하나 — 되돌림 1(도형).** 따르는 줄이 이끄는 줄의 깊이를 원했는데 그
> 깊이가 자기 행에서 이미 찼다. **자리가 진짜로 없는 것**이라 정직한 포기다.
>
